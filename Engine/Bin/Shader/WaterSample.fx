...
Texture2DMS<float4> g_WaterPrevDiffuse : register(t14);
Texture2DMS<float4> g_WaterPrevGBuffer1 : register(t15);
Texture2DMS<float4> g_WaterPrevGBuffer2 : register(t16);
Texture2DMS<float4> g_WaterPrevGBuffer3 : register(t17);
...
PSOutput_GBuffer WaterPS(Vertex3DOutput input)
{
   ...
   //  float penet = pow(dot(input.Normal, input.ViewPos), 5) * 0.2;
    ...
    float4 WaterDiffuseColor = BaseTextureColor;
    ...
    float4 PrevDepth = g_WaterPrevGBuffer2.Load(TargetPos, 0);
    float4 PrevColor = g_WaterPrevGBuffer2.Load(TargetPos, 0);
    ...
    output.Diffuse.rgb = WaterDiffuseColor.rgb; // * (LightInfo.Dif + LightInfo.Amb) + LightInfo.Spc + LightInfo.Emv;
    ...
    output.Diffuse.a = WaterDiffuseColor.a;
    ...
    float4 Refraction = g_WaterPrevGBuffer3.Load(TargetPos + FinalNormal.xy * 0.5f, 0) * 0.5f;

    // output.GBuffer3.a = ConvertColor(ReflectColor * 0.6f * fresnel);
    // output.GBuffer3.a = ConvertColor(EmissiveColor);
    output.GBuffer3.a = ConvertColor(ReflectColor * fresnel * 0.9f);
    // output.GBuffer3.a = ConvertColor(Refraction);

    output.GBuffer4.rgb = input.ViewPos;
    output.GBuffer4.a = 1.f;

    output.GBuffer5.rgb = input.ViewPos;
